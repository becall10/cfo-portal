...
function Attest({ addSubmission }) {
    ...
    const [successMessage, setSuccessMessage] = useState('');  // State to manage the display of the success message

    useEffect(() => {
        // Optionally auto-generate values if needed on component mount or specific trigger
        const autoGeneratedValues = generateData(); // This assumes generateData is defined or imported
        setDataInterfaceDetails(autoGeneratedValues);
    }, []); // Dependency array is empty, meaning this only runs once on mount
    ...
    const handleSubmit = (event) => {
        ...
        // Set success message and then clear it after a delay
        setSuccessMessage("The attestation has been submitted.");
        setTimeout(() => {
            setSuccessMessage('');
...
// Assuming generateData function exists and properly generates initial values
function generateData() {
    return {
        dataSourceCode: `DS-${Math.random().toString(36).substr(2, 9).toUpperCase()}`,
        processTrackId: `PT-${Math.floor(1000 + Math.random() * 9000)}`,
        description: `Sample Description ${Math.floor(100 + Math.random() * 900)}`,
